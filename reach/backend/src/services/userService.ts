...
import { getTelnyxConfigByWorkspace } from '../config/telnyx';
import { phoneArrayToMap } from '../utils/phoneNumberTypes';
...
export const getProfile = async (userId: number) => {
    ...
    let telnyxCredentials;
    let callerIds;

    if (activeWorkspaceId) {
        const telnyxConfig = await getTelnyxConfigByWorkspace(activeWorkspaceId);

        if (telnyxConfig) {
            telnyxCredentials = {
                login: telnyxConfig.username,
                password: telnyxConfig.password,
            };

            const callerIdsMap = phoneArrayToMap(telnyxConfig.callerIds ?? []);
            const smsMap = phoneArrayToMap(telnyxConfig.smsNumbers);
            const primarySms = Object.values(smsMap).flat()[0];
            callerIds =
                Object.keys(callerIdsMap).length > 0
                    ? callerIdsMap
                    : primarySms
                      ? { us: [primarySms] }
                      : {};
    ...
    return {
        ...
        workspaces,
        ...(telnyxCredentials ? { telnyxCredentials, callerIds } : {}),
    };
};
